...
const ResultSearch = () => {
  ...
  const [cancellationFlight, setCancellationFlight] = useState(null);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-center">
                  ...
                  <div className="text-center">
                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 mb-4">
                      <div className="mb-1">
                        <span className="text-3xl font-bold text-green-700">₹{(flight.score || 0).toLocaleString()}</span>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Link
                        to="/result/book"
                        state={{ flight }}
                        className="block w-full bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white font-bold py-3 px-6 rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl text-center"
                      >
                        Book Now
                      </Link>
                      <button
                        onClick={() => setCancellationFlight(flight)}
                        className="text-xs bg-green-100 text-green-800 font-semibold px-3 py-1.5 rounded-lg hover:bg-green-200 transition-colors mt-2 inline-flex items-center gap-1 cursor-pointer"
                      >
                        <span>✓</span> Cancellation Policy & Info
                      </button>
                    </div>
                  </div>
                </div>
        ...
        {cancellationFlight && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl relative animate-fadeIn">
              <button 
                onClick={() => setCancellationFlight(null)}
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{cancellationFlight.airlineName} ({cancellationFlight.flightNumber})</p>
                </div>
              </div>

              <div className="space-y-3 text-sm text-gray-700 border-t border-b border-gray-100 py-4 mb-4">
                <div className="flex justify-between items-center bg-green-50 p-3 rounded-lg">
                  <span className="font-semibold text-green-800">Free Cancellation Window</span>
                  <span className="text-xs bg-green-200 text-green-800 px-2 py-0.5 rounded font-bold">100% Refundable</span>
                </div>
                <ul className="space-y-2 list-disc list-inside text-gray-600">
                  <li>Cancel anytime up to 24 hours prior to departure for a full refund.</li>
                  <li>Instant refund process to your original payment method (UPI / Card).</li>
                  <li>No hidden cancellation fees or penalties.</li>
                </ul>
              </div>

              <button
                onClick={() => setCancellationFlight(null)}
                className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 rounded-xl transition-all shadow-md"
              >
                Got It
              </button>
            </div>
