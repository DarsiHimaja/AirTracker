...
import { getUserBookings, cancelBooking } from '../bookingService';
...
const Profile = () => {
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    setBookingHistory(prev => prev.map(b => b.id === bookingId ? { ...b, status: 'Cancelled (Refund Processed)' } : b));
    toast.success(`Booking ${bookingId} cancelled. 100% refund initiated!`);
  };
              ...
              <div key={booking.id || index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-sm font-semibold ${booking.status?.includes('Cancelled') ? 'text-red-600' : 'text-green-700'}`}>
